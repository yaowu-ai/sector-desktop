...
export function DiagnosticPage() {
  ...
  const [browserDiagnosticAccountId, setBrowserDiagnosticAccountId] =
    useState<string>();
  const [browserDiagnosis, setBrowserDiagnosis] =
  ...
  const runBrowserDiagnosis = async () => {
    ...
    try {
      const diagnosis = await diagnoseAccountBrowser(
        browserDiagnosticAccountId,
      );
    ...
    Modal.confirm({
      ...
      content: (
        ...
        >
          ...
          <Descriptions.Item label="扫描阈值">
            评论数 &gt; {values.minComments}
          </Descriptions.Item>
          <Descriptions.Item label="最大滚动">
            {values.maxScroll}
          </Descriptions.Item>
                ...
                <Tooltip
                  title={
                    browserDiagnosticAccount
                      ? accountDiagnosticDisabledReason(
                          browserDiagnosticAccount,
                        )
                  ...
                  <Descriptions.Item label="浏览器环境">
                    {browserDiagnosticAccount ? (
                      <AccountBrowserEnvironment
                        accounts={[browserDiagnosticAccount]}
                      />
                    ) : (
                      <Typography.Text type="secondary">
                        请选择账号
                ...
                <Row gutter={12}>
                  ...
                  <Col xs={24} md={12}>
                    <Form.Item
                      ...
                      label="最大滚动次数"
                      rules={[
                        { required: true, message: "请输入最大滚动次数" },
                      ]}
                    >
                      <InputNumber
                        min={1}
...
function DiagnosticSummary() {
  ...
  const refresh = useCallback(async () => {
    ...
    if (stdout.content || stderr.content) {
      setOutput((current) =>
        `${current}${stdout.content}${stderr.content}`.slice(-20000),
      );
    }
        ...
        <Descriptions size="small" column={2} bordered>
          ...
          <Descriptions.Item label="状态">
            <StatusTag
              ...
              label={formatProcessStatus(status?.status)}
            />
          </Descriptions.Item>
...
function BrowserDiagnosisResult({
  diagnosis,
}: {
  diagnosis: AccountBrowserDiagnosis | null;
}) {
  if (!diagnosis) {
    return (
      <Alert
        ...
        type="info"
        message="等待浏览器环境诊断"
        description="会检查账号浏览器提供方、可执行文件、代理、用户数据目录和已有 CDP 运行记录。"
      />
    );
  }
  ...
  return (
    <Space direction="vertical" size={12} className="full-width">
      <Descriptions size="small" column={2} bordered>
        ...
        <Descriptions.Item label="浏览器提供方">
          {formatBrowserProvider(diagnosis.provider)}
        </Descriptions.Item>
        <Descriptions.Item label="总体状态">
          <StatusTag
            ...
            label={formatDiagnosisStatus(diagnosis.status)}
          />
        </Descriptions.Item>
      </Descriptions>
      <Table
  ...
  return (
    <Space direction="vertical" size={12} className="full-width">
      <Alert
        ...
        message={
          attempts.length
            ? `已识别 ${attempts.length} 个策略，${changed} 个发生状态变化`
            : "等待点赞诊断输出"
        }
        description={
          output.includes("no active like button")
            ? "当前可视视频没有找到 Like 按钮，请查看 stdout 定位页面状态。"
            : "点击前/点击后状态来自 test_like.py 输出。"
        }
      />
      <Table
        rowKey="strategy"
...
function CommentSummary({
  ...
  return (
    <Space direction="vertical" size={12} className="full-width">
      ...
      />
      <Descriptions size="small" column={2} bordered>
        <Descriptions.Item label="扫描视频数">
          {rows.length || "-"}
        </Descriptions.Item>
        <Descriptions.Item label="最高评论数">
          {maxCommentCount(rows)}
        </Descriptions.Item>
        <Descriptions.Item label="评论输入框">
          {output.includes("[comment-input wrapper]") ? "已输出" : "-"}
        </Descriptions.Item>
        <Descriptions.Item label="发布按钮">
          {output.includes("[comment-post button]") ? "已输出" : "-"}
        </Descriptions.Item>
      </Descriptions>
      <Table
        rowKey="index"
...
const likeColumns: ColumnsType<LikeAttempt> = [
  { title: "策略", dataIndex: "strategy", width: 220 },
  {
    ...
    render: (changed: boolean) => (
      <Tag color={changed ? "green" : "gold"}>
        {changed ? "状态已变化" : "无变化"}
      </Tag>
    ),
  },
  { title: "点击", dataIndex: "click", width: 180, ellipsis: true },
  { title: "点击前", dataIndex: "before", ellipsis: true },
  { title: "点击后", dataIndex: "after", ellipsis: true },
];
...
const browserDiagnosisColumns: ColumnsType<ProviderDiagnosticCheck> = [
  { title: "检查项", dataIndex: "name", width: 190 },
  {
    ...
    render: (status: ProviderDiagnosticCheck["status"]) => (
      <Tag
        color={
          status === "ok" ? "green" : status === "warning" ? "gold" : "red"
        }
      >
        {formatDiagnosisStatus(status)}
      </Tag>
    ),
  },
...
function parseCommentScans(output: string): CommentScanRow[] {
  return Array.from(
    output.matchAll(/video\s+(\d+):\s+comment_count\s+=\s+([^\n]+)/g),
  ).map((match) => ({
...
function parseCommentSummary(output: string) {
  ...
  if (output.includes("NOT POSTED") || output.includes("[FAIL]")) {
    return {
      ...
      message: "评论诊断失败或未发布",
      description:
        "请查看 HTML 和 stderr，重点检查评论数、评论输入框和评论发布按钮选择器。",
    };
  }
  if (
    output.includes("Comment panel HTML") ||
    output.includes("Right-rail HTML")
  ) {
    return {
      posted: false,
      failed: false,
  ...
  return {
    ...
    message: "等待评论诊断输出",
    description:
      "运行后会展示扫描视频数、评论数、选择器 HTML、输入框定位和发布结果。",
  };
}
...
function formatProcessStatus(status?: ProcessStatus["status"]) {
...
function formatDiagnosisStatus(status: ProviderDiagnosticCheck["status"] | AccountBrowserDiagnosis["status"]) {
  if (status === "ok") {
    return "正常";
  }
  if (status === "warning") {
...
function accountCanRunDiagnostics(
