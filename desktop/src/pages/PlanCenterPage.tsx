...
function PlanCard({
  ...
  return (
    <div
      className={
        current
          ? "plan-option-card plan-option-card-current"
          : "plan-option-card"
      }
    >
        ...
        <Space className="plan-option-head">
          ...
          <Space size={6}>
            ...
            <Tag color={plan.status === "active" ? "green" : "default"}>
              {plan.status === "active" ? "可用" : "停用"}
            </Tag>
          </Space>
        </Space>
        <Typography.Text type="secondary">
          套餐价格：{formatPrice(plan.priceCents)}
        </Typography.Text>
        <div className="plan-option-limits">
          {PLAN_QUOTA_ITEMS.map((item) => (
            <div className="plan-option-limit" key={item.key}>
              <Typography.Text type="secondary">{item.label}</Typography.Text>
              <Typography.Text strong>
                {item.type === "number"
                  ? formatLimit(plan.limits?.[item.key])
                  : formatBooleanLimit(plan.limits?.[item.key])}
              </Typography.Text>
            </div>
          ))}
        </div>
