...
export function BrowserProfilePage() {
  ...
  const [batchResult, setBatchResult] =
    useState<BatchCreateProfileResult | null>(null);
  ...
  const bitbrowserAccountCount = useMemo(
    () =>
      accounts.filter((account) => effectiveProvider(account) === "bitbrowser")
        .length,
    [accounts],
  );
  const chromiumAccountCount = useMemo(
    () =>
      accounts.filter(
        (account) => effectiveProvider(account) === "builtin_chromium",
      ).length,
    [accounts],
  );
  ...
  const profileColumns: ColumnsType<BrowserProfile> = [
    ...
    },
    {
      ...
      render: (opened: boolean) => (
        <StatusTag
          status={opened ? "running" : "idle"}
          label={opened ? "已打开" : "未打开"}
        />
      ),
    },
    {
      ...
      width: 150,
      render: (value?: string) =>
        value ? <Tag color="blue">{value}</Tag> : <Tag>未绑定</Tag>,
    },
    {
      title: "代理",
      dataIndex: "proxy",
            ...
            <Row gutter={16}>
              ...
              <Col span={8}>
                <Statistic
                  ...
                  valueStyle={
                    pendingAccountCount > 0 ? { color: "#d97706" } : undefined
                  }
                />
              </Col>
            </Row>
...

              {
                ...
                children: (
                  <BuiltinChromiumPanel
                    ...
                    onDiagnose={(accountId) =>
                      void diagnoseChromiumAccount(accountId)
                    }
          ...
          <Row gutter={12}>
            <Col span={12}>
              <Form.Item name="skipProxyCheck" valuePropName="checked">
                <Switch
                  checkedChildren="跳过检测"
                  unCheckedChildren="检测代理"
                />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="allowUsedProxy" valuePropName="checked">
                <Switch
                  checkedChildren="允许复用"
                  unCheckedChildren="阻止复用"
                />
              </Form.Item>
            </Col>
          </Row>
...
function BatchCreatePanel({
  ...
  const importProxyFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const proxyLines = content.split(/\r?\n/).filter((line) => {
      ...
      <Form form={form} layout="vertical" requiredMark={false}>
        ...
        <Space wrap>
          <Form.Item name="skipProxyCheck" valuePropName="checked" noStyle>
            <Switch
              checkedChildren="跳过代理检测"
              unCheckedChildren="创建前检测代理"
            />
          </Form.Item>
          <Form.Item name="skipUsed" valuePropName="checked" noStyle>
            <Switch
              checkedChildren="跳过已使用代理"
              unCheckedChildren="允许复用代理"
            />
          </Form.Item>
          <Button
            type="primary"
        ...
        {diagnosisResult ? (
          <Descriptions size="small" column={1} bordered>
            <Descriptions.Item label="账号">
              {diagnosisResult.accountId}
            </Descriptions.Item>
            <Descriptions.Item label="Provider">
              {diagnosisResult.provider}
            </Descriptions.Item>
            <Descriptions.Item label="状态">
              <Tag
                color={
                  diagnosisResult.status === "ok"
                    ? "green"
...
function ResultBox({ proxyResult }: { proxyResult: ProxyCheckResult | null }) {
  ...
  return (
    <Alert
      type={
        proxyResult.valid ? (proxyResult.used ? "warning" : "success") : "error"
      }
      showIcon
        ...
        <Alert
          ...
          description={[
            preview.missingProfiles.length
              ...
              : "",
            preview.duplicateProfiles.length
              ? `重名窗口：${preview.duplicateProfiles.join(", ")}`
              : "",
          ]
            .filter(Boolean)
            .join("；")}
      ...
      <Table
        ...
        columns={[
          { title: "账号 ID", dataIndex: "id", width: 150 },
          {
            ...
            dataIndex: "platform",
            width: 110,
            render: (value) => getPlatformLabel(value),
          },
          { title: "IP 组", dataIndex: "ipGroup", width: 90 },
          {
            ...
            width: 120,
            render: (ranges: Array<[number, number]>) =>
...
async function readProxyImportFile(file: File) {
  ...
  if (["doc", "xls"].includes(extension)) {
    throw new Error(
      "暂不支持旧版二进制 .doc/.xls，请另存为 .docx/.xlsx、.csv 或 .txt 后导入",
    );
  }
...
async function readXlsxRows(buffer: ArrayBuffer) {
  ...
  return sheetNames.flatMap((sheetName) =>
    parseWorksheetRows(files.get(sheetName) ?? "", sharedStrings),
  );
}
...
function parseWorksheetRows(xml: string, sharedStrings: string[]) {
  return Array.from(xml.matchAll(/<row[^>]*>([\s\S]*?)<\/row>/g)).map(
    (rowMatch) =>
...
async function readZipFiles(buffer: ArrayBuffer) {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (!name.endsWith("/")) {
      files.set(
        name,
        decodeUtf8(await inflateZipEntry(compressedData, compression)),
      );
    }
...
async function inflateZipEntry(data: Uint8Array, compression: number) {
  ...
  if (!DecompressionStreamCtor) {
    throw new Error(
      "当前 WebView 不支持解压 Office 文件，请改用 .csv 或 .txt 导入",
    );
  }
