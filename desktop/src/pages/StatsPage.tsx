...
export function StatsPage() {
  ...
  const [targetStats, setTargetStats] =
    useState<TargetStatsSummary>(EMPTY_TARGET_STATS);
  ...
  const refresh = async (sourceFilters = filters) => {
    ...
    setLoading(true);
    try {
      const [snapshot, sqlite, nextFypStats, nextTargetStats] =
        await Promise.all([
          loadConfig(),
          getSqliteStatus(),
  ...
  const exportCsv = () => {
    ...
    downloadCsv(
      `account-matrix-stats-${fypStats.scope}-${timestampForFile()}.csv`,
      rows,
    );
  };
        ...
        extra={
          <Space>
            <Button
              ...
              onClick={exportCsv}
              disabled={loading || !licenseLimits.exportCsv}
            >
              导出 CSV
            </Button>
            <Button
      ...
      <Row gutter={[16, 16]}>
          ...
          <Card>
            <Space direction="vertical" size={14} className="full-width">
              <Space wrap size={12}>
                ...
                />
                <Tag color="blue">
                  {formatFypDisplayLabel(displayedFypStats.label)}
                </Tag>
              </Space>
              <Space size={8}>
                <Database size={15} />
                ...
                <Tag color={getStatsStoreStatus(sqliteStatus).color}>
                  {getStatsStoreStatus(sqliteStatus).label}
                </Tag>
              </Space>
            </Space>
          </Card>
        ...
        <Col xs={24} md={8} xl={4}>
          <Card>
            <Statistic
              title="账号数"
              value={displayedFypStats.total.accounts}
        ...
        <Col xs={24} md={8} xl={4}>
          <Card>
            <Statistic
              title="完成"
              value={displayedFypStats.total.ok}
        ...
        <Col xs={24} md={8} xl={4}>
          <Card>
            <Statistic
              title="错误"
              value={displayedFypStats.total.err}
        ...
        <Col xs={24} md={8} xl={4}>
          <Card>
            <Statistic
              title="跳过"
              value={displayedFypStats.total.skip}
        ...
        <Col xs={24} md={8} xl={4}>
          <Card>
            <Statistic
              title="普通视频"
              value={displayedFypStats.total.videos}
                ...
                <Table.Summary fixed>
                  <Table.Summary.Row>
                    ...
                    <Table.Summary.Cell index={1}>
                      {displayedFypStats.total.ok}
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={2}>
                      {displayedFypStats.total.err}
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={3}>
                      {displayedFypStats.total.skip}
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={4}>
                      {displayedFypStats.total.videos}
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={5}>
                      {displayedFypStats.total.likes}
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={6}>
                      {displayedFypStats.total.follows}
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={7}>
                      {displayedFypStats.total.comments}
                    </Table.Summary.Cell>
                  </Table.Summary.Row>
                </Table.Summary>
              )}
...
const fypColumns: ColumnsType<FypAccountStats> = [
  ...
  { title: "完成", dataIndex: "ok", width: 90, sorter: (a, b) => a.ok - b.ok },
  {
    title: "错误",
    dataIndex: "err",
    width: 90,
...
function filterTargetStats(
  ...
  const byHandle = stats.byHandle
    .map((row) => ({
      ...row,
      accounts: row.accounts.filter((accountId) =>
...
function formatFypDisplayLabel(label: string) {
  return label.replace(/\bFYP\b/g, "通用养号任务");
}
...
function buildCsvRows(
  fypStats: FypStatsSummary,
  targetStats: TargetStatsSummary,
) {
  const rows: string[][] = [
    [
      "category",
