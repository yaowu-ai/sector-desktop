...
export function GmailSetupPage() {
  ...
  const runSingle = async () => {
    ...
    confirmRun('单账号 Gmail 初始化', request, [
      ...
      ['新密码', request.newPassword ? '通过环境变量传递' : request.password ? '使用脚本默认新密码' : '未提供'],
    ])
  }
  ...
  const confirmRun = (title: string, request: GmailSetupRequest, rows: Array<[string, string]>) => {
    Modal.confirm({
      title,
      okText: '确认启动',
      cancelText: '取消',
      ...
      content: (
        <Descriptions size="small" column={1} bordered style={{ marginTop: 12 }}>
          ...
          <Descriptions.Item label="页面超时">{request.timeoutSeconds} 秒</Descriptions.Item>
          <Descriptions.Item label="条款页超时">{request.termsTimeoutSeconds} 秒</Descriptions.Item>
        </Descriptions>
      ),
      onOk: async () => {
        setStarting(true)
        try {
          ...
          message.success(`Gmail 初始化脚本已启动，PID ${result.processId ?? '-'}`)
        } catch (error) {
  ...
  return (
    <>
      <PageHeader
        title="Gmail 初始化"
        description={`包装 gmail_setup.py，按 ${currentPlatformDefinition.localeName} 浏览器窗口执行单账号和批量邮箱文件初始化。`}
        extra={
          <Space>
            <Segmented
              value={mode}
      ...
      <Row gutter={[16, 16]}>
        <Col span={24}>
          <Alert
            ...
            type="info"
            message="密码不会进入命令行参数"
            description="单账号模式使用 --password-env 和 --new-password-env 传递密码；批量模式由脚本从本地邮箱文件读取密码。输出面板会对敏感字段做脱敏。"
          />
        </Col>
...
function SingleAccountForm({
  ...
  return (
    <Card
      title="单账号"
      extra={
        <Button type="primary" icon={<Play size={16} />} loading={starting} onClick={onRun}>
          启动
        </Button>
      }
    >
      <Form form={form} layout="vertical" requiredMark={false} initialValues={DEFAULT_SINGLE}>
        <Form.Item name="browserName" label="BitBrowser 窗口名称" rules={[{ required: true, message: '请输入窗口名称' }]}>
          <Input prefix={<Mail size={15} />} placeholder={`例如 ${platformPrefix}_25`} />
        </Form.Item>
        ...
        <Form.Item name="password" label="当前密码">
          <Input.Password prefix={<KeyRound size={15} />} placeholder="通过环境变量传给脚本" autoComplete="off" />
        </Form.Item>
        <Form.Item name="newPassword" label="新密码">
          <Input.Password prefix={<KeyRound size={15} />} placeholder="为空时脚本按自身默认策略处理" autoComplete="new-password" />
        </Form.Item>
        <SharedFields />
      </Form>
    </Card>
...
function BatchFileForm({
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" requiredMark={false} initialValues={DEFAULT_BATCH}>
        <Form.Item name="browserName" label="起始 BitBrowser 窗口名称" rules={[{ required: true, message: '请输入起始窗口名称' }]}>
          <Input prefix={<Mail size={15} />} placeholder={`例如 ${platformPrefix}_25，后续自动递增`} />
        </Form.Item>
        <Form.Item
          name="emailFile"
          label="邮箱文件"
        ...
        >
          <Input
            ...
            addonAfter={
              <Button type="text" size="small" icon={<FolderOpen size={14} />} onClick={onChooseFile}>
                选择
              </Button>
            }
          />
        </Form.Item>
        <Form.Item name="keepOpenOnError" label="失败时保留窗口" valuePropName="checked">
...
function SharedFields() {
  return (
    <Row gutter={12}>
      <Col span={24}>
        <Form.Item name="query" label="搜索词" rules={[{ required: true, message: '请输入搜索词' }]}>
...
function GmailRunStatus() {
  ...
  const refresh = useCallback(async () => {
    ...
    if (stdout.content || stderr.content) {
      setRecentOutput((current) => `${current}${stdout.content}${stderr.content}`.slice(-8000))
    }
    ...
    >
      <Space direction="vertical" size={14} className="full-width">
        <Descriptions size="small" column={1} bordered>
          <Descriptions.Item label="任务">{gmailActive ? 'Gmail 初始化' : status?.taskType ?? '-'}</Descriptions.Item>
          <Descriptions.Item label="状态">
            <StatusTag status={derived.tone} label={derived.label} />
          </Descriptions.Item>
          <Descriptions.Item label="PID">{status?.processId ?? '-'}</Descriptions.Item>
          <Descriptions.Item label="开始时间">{status?.startedAt ?? '-'}</Descriptions.Item>
          <Descriptions.Item label="结束时间">{status?.endedAt ?? '-'}</Descriptions.Item>
        </Descriptions>
        <Alert showIcon type={derived.alertType} message={derived.message} description={derived.description} />
        <Space wrap>
          <Tag>额外验证会保留窗口</Tag>
          <Tag>密码环境变量传递</Tag>
          <Tag>日志默认脱敏</Tag>
...
function deriveGmailStatus(status: ProcessStatus | null, output: string) {
  ...
  if (needsManual) {
    return {
      ...
      description: '请在 BitBrowser 当前窗口完成验证或检查页面状态，再根据输出决定是否重试。',
    }
  }
  ...
  if (status.status === 'running' || status.status === 'starting') {
    return {
      ...
      description: '步骤日志会持续刷新，遇到 Google 额外验证时窗口会停留以便人工处理。',
    }
  }
