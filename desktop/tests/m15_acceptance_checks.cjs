...
assertContains(platformSelector, 'filterPlatformsByRole(PLATFORMS, userRole)', 'top platform selector should filter platforms by user role')
assertContains(platformSelector, 'value={selectedPlatform}', 'top platform selector should bind a role-visible platform')
assertContains(platformSelector, 'setCurrentPlatform(selectedPlatform)', 'top platform selector should leave hidden cached platforms')
assertContains(appShell, 'userRole={userRole}', 'app shell should pass product user role to platform selector')
assertContains(appShell, 'canOpenSettings={isTechnician}', 'platform settings entry should remain technician-only')
...
assert.equal(appShell.includes('items={siderMenuItems}'), true, 'left navigation should use permitted main route menu items')
