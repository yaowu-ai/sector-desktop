...
const schedulerRs = read(desktopRoot, 'src-tauri', 'src', 'commands', 'scheduler.rs')
...
const cliPy = read(projectRoot, 'src', 'platforms', 'instagram_runner', 'cli.py')
const runtimeCliPy = read(projectRoot, 'src', 'runtime_cli.py')
const pyinstallerSpec = read(projectRoot, 'runtime', 'pyinstaller', 'account-matrix-runtime.spec')
...
assertContains(api, "invoke<SchedulerStartResult>('start_scheduler', { platform })", 'scheduler API should pass current platform')
assertContains(api, "invoke<SchedulerHealth>('get_scheduler_health', { platform })", 'scheduler health API should pass current platform')
...
assertContains(schedulerRs, 'INSTAGRAM_PLATFORM', 'scheduler backend should define an Instagram scheduler branch')
assertContains(schedulerRs, '"platforms.instagram_runner"', 'source scheduler command should launch Instagram runner')
assertContains(schedulerRs, '"instagram-scheduler"', 'bundled scheduler command should launch the Instagram runtime command')
assertContains(schedulerRs, 'instagram_schedule_state_path', 'Instagram scheduler health should read schedule_state.json')
assertContains(schedulerRs, 'detect_ip_group_conflicts(&platform)', 'scheduler conflict checks should be scoped by platform')
...
assertContains(bridgePy, '_patch_schedule_state_module', 'Python bridge should patch Instagram schedule state into app data')
assertContains(bridgePy, 'scheduler_job_runs', 'Python bridge should mirror Instagram schedule events into desktop run history')
assertContains(cliPy, 'parser.add_argument("--schedule"', 'Instagram CLI should expose original schedule mode')
assertContains(cliPy, 'ins_cli._run_schedule', 'Instagram CLI should reuse the original long-running scheduler')
assertContains(cliPy, 'account.get("scheduled", True)', 'Instagram scheduler should only load scheduled accounts')
assertContains(runtimeCliPy, '"instagram-scheduler"', 'bundled runtime should expose Instagram scheduler command')
assertContains(pyinstallerSpec, 'collect_submodules("ins")', 'runtime package should include the standalone ins package')
